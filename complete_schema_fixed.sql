-- ONEGO Learning - Complete Database Schema (Fixed)
...
-- Create subscription plans enum (only if it doesn't exist)
DO $$ 
BEGIN
...
-- Create course assignments table (only if it doesn't exist)
...
-- Create user performance table (only if it doesn't exist)
...
-- Create subscribers table (only if it doesn't exist)
...
-- Create honest box tables (only if they don't exist)
CREATE TABLE IF NOT EXISTS public.honest_box_monthly_questions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS public.honest_box_feedback (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
  group_id UUID REFERENCES public.groups(id) ON DELETE CASCADE NOT NULL,
...
-- Create website data table (only if it doesn't exist)
...
-- Create session feedback table (only if it doesn't exist)
...
-- Create course usage tracking table (only if it doesn't exist)
...
-- Create active sessions table (only if it doesn't exist)
...
-- Create individual course assignments table (only if it doesn't exist)
CREATE TABLE IF NOT EXISTS public.individual_course_assignments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
  course_id UUID REFERENCES public.courses(id) ON DELETE CASCADE NOT NULL,
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'team') THEN
    ALTER TABLE public.profiles ADD COLUMN team TEXT[] DEFAULT ARRAY['General'];
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'status') THEN
    ALTER TABLE public.courses ADD COLUMN status TEXT DEFAULT 'drafted';
  END IF;
END $$;
...
-- Add foreign key constraint for group_id in profiles (only if it doesn't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_profiles_group') THEN
...
-- Add foreign key constraints to individual_course_assignments table (only if they don't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'individual_course_assignments_course_id_fkey') THEN
    ALTER TABLE public.individual_course_assignments
    ADD CONSTRAINT individual_course_assignments_course_id_fkey
    FOREIGN KEY (course_id) REFERENCES public.courses(id) ON DELETE CASCADE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'individual_course_assignments_user_id_fkey') THEN
    ALTER TABLE public.individual_course_assignments
    ADD CONSTRAINT individual_course_assignments_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'individual_course_assignments_assigned_by_fkey') THEN
    ALTER TABLE public.individual_course_assignments
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_group_id') THEN
    CREATE INDEX idx_profiles_group_id ON public.profiles(group_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_courses_creator_id') THEN
    CREATE INDEX idx_courses_creator_id ON public.courses(creator_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_course_assignments_group_id') THEN
    CREATE INDEX idx_course_assignments_group_id ON public.course_assignments(group_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_performance_user_id') THEN
    CREATE INDEX idx_user_performance_user_id ON public.user_performance(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_performance_course_id') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_invitations_magic_link_token') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_subscribers_email') THEN
    CREATE INDEX idx_subscribers_email ON public.subscribers(email);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_honest_box_feedback_group_id') THEN
    CREATE INDEX idx_honest_box_feedback_group_id ON public.honest_box_feedback(group_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_website_data_group_id') THEN
    CREATE INDEX idx_website_data_group_id ON public.website_data(group_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_individual_course_assignments_user_id') THEN
    CREATE INDEX idx_individual_course_assignments_user_id ON public.individual_course_assignments(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_individual_course_assignments_course_id') THEN
...
DROP POLICY IF EXISTS "Admins can manage their groups" ON public.groups;
...
DROP POLICY IF EXISTS "Admins can manage their courses" ON public.courses;
...
-- Drop existing course assignment policies
DROP POLICY IF EXISTS "Admins can manage course assignments" ON public.course_assignments;
DROP POLICY IF EXISTS "Users can view their assignments" ON public.course_assignments;
...
-- Drop existing user performance policies
DROP POLICY IF EXISTS "Users can manage their own performance" ON public.user_performance;
DROP POLICY IF EXISTS "Admins can view their team's performance" ON public.user_performance;
...
DROP POLICY IF EXISTS "Users can view invitations for their group" ON public.invitations;
...
-- Drop existing subscriber policies
...
-- Drop existing honest box policies
DROP POLICY IF EXISTS "Users can view honest box questions for their group" ON public.honest_box_monthly_questions;
DROP POLICY IF EXISTS "Users can submit honest box feedback" ON public.honest_box_feedback;
DROP POLICY IF EXISTS "Users can view honest box feedback for their group" ON public.honest_box_feedback;
...
-- Drop existing website data policies
DROP POLICY IF EXISTS "Users can manage website data for their group" ON public.website_data;
...
-- Drop existing session feedback policies
DROP POLICY IF EXISTS "Users can manage their own session feedback" ON public.session_feedback;

-- RLS Policies for session feedback
CREATE POLICY "Users can manage their own session feedback" ON public.session_feedback
  FOR ALL USING (user_id = auth.uid());

-- Drop existing course usage policies
DROP POLICY IF EXISTS "Users can manage their own course usage" ON public.course_usage;

-- RLS Policies for course usage
CREATE POLICY "Users can manage their own course usage" ON public.course_usage
  FOR ALL USING (user_id = auth.uid());

-- Drop existing active sessions policies
...
-- Drop existing individual course assignment policies
DROP POLICY IF EXISTS "Users can view their own assignments" ON public.individual_course_assignments;
DROP POLICY IF EXISTS "Admins can view assignments in their group" ON public.individual_course_assignments;
DROP POLICY IF EXISTS "Admins can create assignments for their group members" ON public.individual_course_assignments;
DROP POLICY IF EXISTS "Admins can update assignments for their group members" ON public.individual_course_assignments;
DROP POLICY IF EXISTS "Admins can delete assignments for their group members" ON public.individual_course_assignments;
...
DROP TRIGGER IF EXISTS on_profile_created_subscriber ON public.profiles;
...
DROP TRIGGER IF EXISTS on_invitation_created_license ON public.invitations;
DROP TRIGGER IF EXISTS on_invitation_deleted_license ON public.invitations;
